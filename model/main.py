...
def train(esa, train_data, label, criterion, optimizer, n_epoch, save_every, directory, device, clip, entity2vec, pred2ix, regularization):
...
def train_iter(db_name, base, data, label, pred2ix, pred2ix_size, entity2vec, \
            ...
            criterion, clip, lr, \
            n_epoch, save_every, regularization, method, device):
    if regularization == True:
        print("use regularization in training")
    for i in range(5):
        train_data, train_label, test_data, test_label = utils.split_data(base, i, data, label)
        esa = model.ESA(pred2ix_size, pred_embedding_dim, transe_dim, hidden_size, method, device)
        esa.to(device)
...
def generator(DB_NAME, base, data, label, entity2vec, pred2ix, pred2ix_size, \
    ...
    for num in tqdm(range(5)):
        ...
        esa = model.ESA(pred2ix_size, args.pred_embedding_dim, args.transE_dim, hidden_size, method, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, default="general", help="attention methods: dot, general or original")
    ...
    if args.mode == "train" or args.mode == "all":
        ...
        train_iter(args.DB_NAME, base, data, label, pred2ix, pred2ix_size, entity2vec, \
                ...
                criterion, args.clip, args.lr, args.n_epoch, args.save_every, args.regularization, args.method, device)
